...
app.get('/api/dashboard',(req,res)=>{
 try {
  const totalLivros=db.prepare('SELECT COALESCE(SUM(exemplares),0) n FROM livros').get().n;
  const disponiveis=db.prepare('SELECT COALESCE(SUM(disponiveis),0) n FROM livros').get().n;
  const leitores=db.prepare('SELECT COUNT(*) n FROM leitores').get().n;
  const ativos=db.prepare("SELECT COUNT(*) n FROM emprestimos WHERE status='aberto'").get().n;
  const atrasados=db.prepare("SELECT COUNT(*) n FROM emprestimos WHERE status='aberto' AND data_prevista < date('now','localtime')").get().n;
  const recentes=db.prepare(`SELECT e.id,e.data_emprestimo,e.data_prevista,e.data_devolucao,e.status,l.titulo,le.nome leitor
    FROM emprestimos e JOIN livros l ON l.id=e.livro_id JOIN leitores le ON le.id=e.leitor_id ORDER BY e.criado_em DESC LIMIT 8`).all();
  res.json({totalLivros,disponiveis,leitores,ativos,atrasados,recentes});
 } catch(e){ console.error(e); res.status(500).json({error:'Erro ao carregar painel.'}); }
});
...
app.post('/api/livros',(req,res)=>{
 const titulo=s(req.body?.titulo), autor=s(req.body?.autor), isbn=s(req.body?.isbn), categoria=s(req.body?.categoria), editora=s(req.body?.editora), ano=safeInt(req.body?.ano,0), exemplares=Math.max(1,safeInt(req.body?.exemplares,1));
 if(!titulo || !autor) return res.status(400).json({error:'Título e autor são obrigatórios.'});
 try { const novo=id(); db.prepare('INSERT INTO livros(id,titulo,autor,isbn,categoria,editora,ano,exemplares,disponiveis) VALUES(?,?,?,?,?,?,?,?,?)').run(novo,titulo,autor,isbn,categoria,editora,ano||null,exemplares,exemplares); res.status(201).json(db.prepare('SELECT * FROM livros WHERE id=?').get(novo)); } catch(e){ console.error(e); res.status(500).json({error:'Não foi possível cadastrar o livro.'}); }
});
