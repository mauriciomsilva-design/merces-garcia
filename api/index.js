...
function rateLimit(req, res, next) {
  ...
  current.count += 1;
  if (current.count > RATE_MAX) return res.status(429).json({ error: 'Muitas requisições. Tente novamente em instantes.' });
  return next();
}
app.use(rateLimit);
...
function normalizarQRCode(valor) { return String(valor ?? '').trim(); }
...
function requireSameOrigin(req, res, next) {
  if (!isAllowedOrigin(req)) return res.status(403).json({ error: 'Origem da requisição não autorizada.' });
  return next();
}
...
function logError(route, err) {
  console.error(`${route}:`, { message: err?.message, code: err?.code, status: err?.status, details: err?.details });
}
...
app.get('/api/alunos', requireSameOrigin, async (req, res) => {
  try {
    const supabase = getSupabase();
    const { data, error } = await supabase.from('alunos').select('id,nome,turma,fone,qrcode,cafe,almoco').order('nome', { ascending: true });
    if (error) throw error;
...
app.get('/api/historico', requireSameOrigin, async (req, res) => {
  try {
    const supabase = getSupabase();
    const { data, error } = await supabase.from('historico').select('id,aluno_id,nome,turma,data,hora,cafe,almoco').order('criado_em', { ascending: false });
    if (error) throw error;
    return res.json(data || []);
  } catch (err) {
...
app.post('/api/alunos', requireSameOrigin, async (req, res) => {
  ...
  try {
    const supabase = getSupabase();
    const { data: existente, error: buscaError } = await supabase.from('alunos').select('id,nome,turma,qrcode').eq('qrcode', qrcode).maybeSingle();
    if (buscaError) throw buscaError;
    if (existente) return res.status(409).json({ error: `Este QR Code já está vinculado ao aluno ${existente.nome}.`, codigo: 'QR_DUPLICADO', aluno: existente });

    const { data: aluno, error: insertError } = await supabase.from('alunos').insert({ nome, turma, fone, qrcode, cafe, almoco }).select('id,nome,turma,fone,qrcode,cafe,almoco').single();
    if (insertError) {
      if (insertError.code === '23505') return res.status(409).json({ error: 'Este QR Code já está cadastrado.', codigo: 'QR_DUPLICADO' });
      throw insertError;
    }
...
app.delete('/api/alunos/:id', requireSameOrigin, async (req, res) => {
  ...
  if (!id || id.length > 100 || !/^[A-Za-z0-9_-]+$/.test(id)) return res.status(400).json({ error: 'Identificador de aluno inválido.' });
  try {
    const supabase = getSupabase();
    const { data, error } = await supabase.from('alunos').delete().eq('id', id).select('id').maybeSingle();
    if (error) throw error;
...
app.post('/api/presenca', requireSameOrigin, async (req, res) => {
  ...
  try {
    const supabase = getSupabase();
    const { data: aluno, error: alunoError } = await supabase.from('alunos').select('id,nome,turma,fone,qrcode').eq('qrcode', qrcode).maybeSingle();
    ...
    const { data: historico, error: histError } = await supabase.from('historico').insert({ aluno_id: aluno.id, nome: aluno.nome, turma: aluno.turma, fone: aluno.fone, qrcode: aluno.qrcode, data, hora, cafe, almoco }).select('id,aluno_id,nome,turma,data,hora,cafe,almoco').single();
...
app.get('/api/config/foto', requireSameOrigin, async (req, res) => {
  try {
    const supabase = getSupabase();
...
// Never expose Express/CORS errors or stack traces to clients.
app.use((err, req, res, next) => {
  if (err?.message === 'CORS origin not allowed') return res.status(403).json({ error: 'Origem da requisição não autorizada.' });
